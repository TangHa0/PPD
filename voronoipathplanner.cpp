...
#define MAX_EDGES 1000
    ...
    std::vector<int> vronoi_indecies(vertexs, -1);
    for (int i = 0; i < vertexs; ++i)
    {
    ...
    for (int i = 0; i < edges; ++i)
    {
        if (!(vronoi_indecies[edges_s[i]] < 0 || vronoi_indecies[edges_t[i]] < 0))
        {
            weight_matrix[vronoi_indecies[edges_s[i]]][vronoi_indecies[edges_t[i]]] =
                    weight_matrix[vronoi_indecies[edges_t[i]]][vronoi_indecies[edges_s[i]]] =
                    distance(voronoi_points[vronoi_indecies[edges_s[i]]], voronoi_points[vronoi_indecies[edges_t[i]]]);
